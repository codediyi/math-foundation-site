...
> 把每个数学概念连接到模型对象、论文线和最小练习。

## 学习目标

- 能按概念回查 Transformer 对应对象。
- 能知道读某类论文前补哪些数学。
- 能把学习内容转成练习。

## 这一章为什么重要

映射表防止数学学习失焦。每个概念都必须回答：它对应哪个张量、哪个公式、哪个训练现象或哪类论文。
...
若要读 FlashAttention，先补 softmax、log-sum-exp、分块矩阵乘法和显存访问，而不是先读完整 CUDA 文档。
...
```python
concept = {"SVD": ["low-rank", "KV compression", "rank-k experiment"]}
print(concept)
```
...
这五步会把 数学概念到 Transformer 映射表 从抽象概念变成可检查的模型行为。
...
学习 数学概念到 Transformer 映射表 时，至少达到“可实现”层级再进入下一章。若目标是论文研究，需要达到“可诊断”或“可研究”。
...
数学概念到 Transformer 映射表 不是孤立章节。你应该主动回看这些关系：
...
1. 数学概念到 Transformer 映射表 最重要的一个数学对象是什么？
