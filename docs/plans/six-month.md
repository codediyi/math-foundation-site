...
> 从数学基础到 Transformer 最小研究能力的 26 周计划。
...
没有周计划，数学学习容易失焦。六个月路线把概念、推导、实现和实验绑定，确保每阶段都能转化为研究能力。
...
\[
10h = 3h\text{阅读}+2h\text{推导}+3h\text{实现}+2h\text{复盘}.
\]
...
| 项目 | 内容 |
|---|---|
| 线代阶段 | 低秩近似和 einsum attention |
| 概率阶段 | logits 缩放和稳定 softmax |
| 微分阶段 | attention/LN 反传 |
| Transformer 阶段 | mini block 和技术报告 |


## 逐步例题

第 5 周学习 SVD：阅读 SVD，手推低秩直觉，写 rank-k 压缩代码，输出误差曲线。
...
```python
weeks = {"1-6": "linear algebra", "7-12": "probability", "13-17": "optimization", "18-26": "transformer"}
print(weeks)
```
...
六个月结束时产出 mini Transformer 代码、技术报告和实验表。
...
学习 六个月路线 时，至少达到“可实现”层级再进入下一章。若目标是论文研究，需要达到“可诊断”或“可研究”。
