...
资源越多越容易失焦。好的资源索引不是链接堆，而是告诉你何时使用、用来解决什么问题、产出什么。
...
\[
\text{资源价值}=\text{解决当前问题的程度}\times\text{可产出程度}.
\]

不能转成笔记、推导或实验的资源，短期价值较低。
...
|---|---|
| D2L | attention 和优化实现 |
| Boyd | 凸优化和 smoothness |
| ProbML | 概率和统计建模 |
| Matrix Cookbook | 矩阵微分速查 |


## 逐步例题

学 LayerNorm 时，先读本课程归一化页，再看 D2L/PyTorch 实现，最后写 autograd 验证，而不是搜索十篇博客。
...
```python
resources = ["D2L", "ProbML", "Boyd EE364a", "Matrix Cookbook"]
...
学习 资源索引 时，至少达到“可实现”层级再进入下一章。若目标是论文研究，需要达到“可诊断”或“可研究”。
