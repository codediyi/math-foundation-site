...
> 用熵、交叉熵和 KL 理解训练目标、蒸馏和 attention 熵。
...
语言模型训练目标、知识蒸馏、RLHF 约束、attention 熵和采样温度都离不开信息论。它提供了衡量分布差异和不确定性的语言。
...
- KL 散度。
- 互信息。
- temperature 和 entropy regularization。
...
| 交叉熵 | 语言模型训练损失 |
| KL | 蒸馏、RLHF、分布约束 |
| 熵 | attention 集中度和采样不确定性 |
| 温度 | logits 缩放和概率尖锐程度 |


## 逐步例题

固定 logits，降低温度会让分布更尖锐、熵更低；升高温度会让分布更平滑、熵更高。
...
- 证明交叉熵分解。
- 比较 forward KL 和 reverse KL 的直觉差异。
- 画 temperature 与 entropy 的曲线。
...
一页笔记：交叉熵、KL、NLL 和蒸馏损失之间的关系。

## 学习路径衔接

之后读“概率统计”和“高效 Attention”。
...
学习 信息论 时，至少达到“可实现”层级再进入下一章。若目标是论文研究，需要达到“可诊断”或“可研究”。
