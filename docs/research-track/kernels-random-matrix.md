...
> 理解 softmax kernel 近似、随机特征和初始化谱。

## 学习目标

- 掌握 kernel 和随机特征直觉。
- 理解随机矩阵谱与初始化尺度。
- 能连接 Performer、\(\mu P\) 和稳定性。

## 这一章为什么重要

高效 attention 与训练尺度理论都绕不开 kernel 和随机矩阵。前者近似相似度函数，后者分析宽度、谱和方差传播。
...
若能找到低维 \(\phi\)，就可能降低 attention 的显式二次计算。
...
对不同宽度随机矩阵画奇异值，观察谱尺度是否随宽度变化。这是理解参数化尺度的第一步。
...
- 认为 kernel 方法只是传统机器学习。
- 忽略随机特征近似误差。
- 把谱范数和 Frobenius 范数混用。
- 不按参数化规则比较宽度。

## 最小练习

- 实现一个 RBF kernel 的随机特征近似。
- 画不同宽度随机矩阵奇异值。
- 解释 Performer 的线性化来自哪里。
...
一页笔记：kernel 近似和随机矩阵如何进入 Transformer。
...
这五步会把 核方法与随机矩阵 从抽象概念变成可检查的模型行为。
...
学习 核方法与随机矩阵 时，至少达到“可实现”层级再进入下一章。若目标是论文研究，需要达到“可诊断”或“可研究”。
