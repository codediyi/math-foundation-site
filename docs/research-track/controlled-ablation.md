...
- 能写出结构改进假设。
- 能设计 baseline、控制变量和指标。
- 能分析失败来源。

## 这一章为什么重要

没有 controlled ablation，结构改进很难形成可信结论。性能变化可能来自参数量、训练步数、实现细节或随机性，而不是新结构本身。
...
\[
\text{effect}=\text{metric}_{\text{new}}-\text{metric}_{\text{baseline}}.
\]

只有在控制变量清楚时，这个差值才有解释力。
...
比较 RoPE 和 ALiBi 时，固定模型大小、数据、训练步数和优化器，只改变位置机制。报告训练长度内指标和长度外推指标。
...
```python
plan = {
 "hypothesis": "ALiBi improves length extrapolation",
 "baseline": "RoPE",
 "fixed": ["model", "data", "steps", "optimizer"],
 "metrics": ["loss", "long_context_accuracy"]
}
print(plan)
```
...
- 为 Pre-LN vs Post-LN 写 ablation 表。
- 为 KV cache 压缩设计评估指标。
- 列出一次实验失败的四类可能原因。
...
学习 结构改进实验 时，至少达到“可实现”层级再进入下一章。若目标是论文研究，需要达到“可诊断”或“可研究”。
