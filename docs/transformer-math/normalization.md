...
> 用尺度控制理解深层 Transformer 的训练稳定性。

## 学习目标

- 理解 residual、LayerNorm、RMSNorm、Pre-LN、Post-LN。
- 能解释残差 Jacobian。
- 能写 LayerNorm 最小实现。

## 这一章为什么重要

深层模型能训练起来，不只因为参数多，还因为残差和归一化提供了稳定的信号传播路径。归一化位置经常决定训练早期是否稳定。
...
把输入乘以 10 后，LayerNorm 输出仍近似零均值单位方差。这说明它直接约束子层输入尺度。
...
print(layer_norm(x).mean(-1), layer_norm(x).var(-1, unbiased=False))
```
...
- 认为归一化只是加速收敛的小技巧。
- 混淆 batch norm 和 layer norm。
- 忽略 eps 对数值稳定的影响。
- 只比较最终 loss，不看训练早期梯度。

## 最小练习

- 手写 LayerNorm 并与 PyTorch 对比。
- 画 Pre-LN/Post-LN block 图。
- 记录不同归一化位置的 grad norm。
...
一页笔记：残差、归一化和训练稳定性的关系。

## 学习路径衔接

之后读“优化与数值稳定”和“结构改进实验”。
...
学习 残差与归一化 时，至少达到“可实现”层级再进入下一章。若目标是论文研究，需要达到“可诊断”或“可研究”。
