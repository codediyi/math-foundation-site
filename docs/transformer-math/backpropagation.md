...
- 理解 attention backward 的主路径。
- 能手推 \(O=PV\) 和 \(S=QK^\top\) 的梯度。
- 能解释 softmax 饱和对梯度的影响。

## 这一章为什么重要

很多 attention 改动改变 logits 或概率分布，进而改变梯度。理解反传路径是判断结构稳定性的前提。
...
\[
O=PV:\quad G_P=G_OV^\top,\quad G_V=P^\top G_O.
\]

\[
S=QK^\top/\sqrt d:\quad G_Q=G_SK/\sqrt d,\quad G_K=G_S^\top Q/\sqrt d.
\]
...
| G_P | 路由权重学习 |
| G_Q/G_K | 打分空间学习 |
| softmax backward | 饱和后的梯度衰减 |


## 逐步例题

若 logits 极大，softmax 接近 one-hot，非最大位置的梯度很小。temperature、缩放和归一化都会影响这个现象。
...
```python
import torch
logits = torch.tensor([[1., 2., 20.]], requires_grad=True)
p = torch.softmax(logits, dim=-1)
loss = p[0, 0]
loss.backward()
print(p.detach(), logits.grad)
```
...
- 只看前向 attention，不看梯度。
- 忘记 mask 位置不会收到有效梯度。
- 把 softmax 饱和误判为模型很自信。
- 推导时漏掉 \(1/\sqrt d\)。

## 最小练习

- 手推 \(O=PV\) 反传。
- 用 autograd 验证 \(QK^\top\) 反传。
- 比较不同 logits 尺度下 softmax 梯度。
...
一张 attention backward 流程图。

## 学习路径衔接

之后读“矩阵微分”和“高效 Attention”。
...
这五步会把 Attention 反向传播 从抽象概念变成可检查的模型行为。
...
学习 Attention 反向传播 时，至少达到“可实现”层级再进入下一章。若目标是论文研究，需要达到“可诊断”或“可研究”。
...
Attention 反向传播 不是孤立章节。你应该主动回看这些关系：
...
1. Attention 反向传播 最重要的一个数学对象是什么？
