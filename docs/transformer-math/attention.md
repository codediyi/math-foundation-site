...
> 从 Q/K/V 到 logits、softmax 和 value 聚合。

## 学习目标

- 能写出 attention 每一步形状。
- 能解释 mask 和 \(1/\sqrt{d_k}\)。
- 能估算 attention 显存复杂度。

## 这一章为什么重要

Attention 是 Transformer 的信息路由机制。每个 token 通过 query 去读取 key/value 中的信息，路由权重由 softmax 决定。它也是长上下文最大的二次复杂度来源。
...
\[
\operatorname{Attention}(Q,K,V)=\operatorname{softmax}\left(\frac{QK^\top}{\sqrt{d_k}}+M
\right)V.
\]
...
设 \(B=2,H=4,N=128,D=64\)。logits 形状为 \(2,4,128,128\)，元素数为 131072。上下文长度翻倍，logits 元素数变为四倍。
...
- 写出 B,H,N,D 下所有中间张量形状。
- 实现 causal mask。
- 比较缩放和不缩放的 attention 熵。
...
一张 attention 数据流图，标注 shape、复杂度和梯度路径。

## 学习路径衔接

之后读“多头注意力”和“Attention 反向传播”。
...
这五步会把 Attention 形状流 从抽象概念变成可检查的模型行为。
...
学习 Attention 形状流 时，至少达到“可实现”层级再进入下一章。若目标是论文研究，需要达到“可诊断”或“可研究”。
...
Attention 形状流 不是孤立章节。你应该主动回看这些关系：
