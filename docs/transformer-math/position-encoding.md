...
> 用位置机制让 attention 感知顺序和相对距离。

## 学习目标

- 区分绝对位置、相对位置、RoPE 和 ALiBi。
- 能写出 RoPE 的旋转直觉。
- 能设计位置外推 toy 实验。

## 这一章为什么重要

Self-attention 本身对顺序不敏感。位置机制决定模型如何表达距离、方向和长度外推，是长上下文研究的核心入口。
...
| sinusoidal | 固定频率位置特征 |
| RoPE | Q/K 旋转和相对位置 |
| ALiBi | attention 距离偏置 |
| LongRoPE | 频率重标定和外推 |


## 逐步例题

二维向量旋转中，位置 m 和 n 的内积只依赖角度差。RoPE 将这个想法扩展到 hidden 维的成对通道。
...
- 实现 sinusoidal、RoPE、ALiBi toy 版本。
- 比较距离 bias 随位置差变化。
- 设计长度翻倍的 copy toy task。
...
学习 位置编码 时，至少达到“可实现”层级再进入下一章。若目标是论文研究，需要达到“可诊断”或“可研究”。
